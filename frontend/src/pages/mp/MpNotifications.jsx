    ...
    <div className="space-y-6 max-w-4xl mx-auto h-full flex flex-col">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Stay updated on your projects and AI risk alerts.</p>
        </div>
        <button 
      ...
      <div className="bg-white p-2 rounded-xl shadow-sm border border-slate-200 flex flex-wrap gap-2">
        {['All', 'Unread', 'warning', 'success', 'info'].map(f => (
          <button 
            key={f}
            onClick={() => setFilter(f)}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-1">
                    <p className={`text-base ${!notif.read ? 'font-bold text-[#123b63]' : 'font-semibold text-slate-700'}`}>
                      {notif.title || (notif.type === 'warning' ? 'AI Risk Alert' : notif.type === 'success' ? 'Update' : 'Information')}
                    </p>
