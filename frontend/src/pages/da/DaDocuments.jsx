...
const DaDocuments = () => {
  ...
  useEffect(() => {
    daApi.getProjects().then(projects => {
  ...
  const filteredDocs = documents.filter(doc => 
    ...
    doc.mpName.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="space-y-6 max-w-7xl mx-auto h-full flex flex-col">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Access all project documents across the district.</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col sm:flex-row gap-4 items-center">
        <div className="flex-1 relative w-full">
          ...
          <input 
            type="text" 
            placeholder="Search by document name, project, or MP..."
            value={searchTerm}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/80 border-b border-slate-200 text-slate-500 text-xs uppercase tracking-wider">
                  ...
                  <th className="p-4 font-semibold hidden lg:table-cell">Associated Project</th>
                  <th className="p-4 font-semibold hidden md:table-cell">MP</th>
                ...
                {filteredDocs.map(doc => (
                  <tr key={doc.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="p-4 hidden md:table-cell">
                      <p className="text-sm text-slate-700">{doc.mpName}</p>
                    </td>
